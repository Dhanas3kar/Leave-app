...
import { requireEmployee, requireManager, authorizeLeaveOwnership, toSafeAuthError } from '../lib/authorization';
...
export async function cancelLeaveRequest(requestId: string) {
  try {
    ...
    // IDOR Protection: Verify ownership before proceeding
    await authorizeLeaveOwnership(requestId, prisma);
